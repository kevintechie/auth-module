...
export default class Oauth2Scheme {
  ...
  async refreshTokens () {
    ...
    const newToken = this.$auth.addTokenPrefix(response.data[this.options.token_key])
    const newRefreshToken = this.$auth.addTokenPrefix(response.data[this.options.refresh_token_key])
